...
type TheoryCoachFormValues = z.infer<typeof formSchema>;

interface TheoryCoachProps {
...
export function TheoryCoach({ initialTopic }: TheoryCoachProps) {
  const { toast } = useToast();
  const { user } = useProMode();
  const { execute: runGenerateAnswer, data: generatedAnswer, isLoading, error, reset } = useAiAgent(generateTheoryAnswer, {
  ...
  const form = useForm<TheoryCoachFormValues>({
